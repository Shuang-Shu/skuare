...
- `skr build <skillName> [refSkill...] [--all]`：为本地 skill 自动创建或追加更新依赖文件（`skill-deps.json` / `skill-deps.lock.json`），支持 `alias=refSkill`；`--all` 会将当前目录下全部合法 skillDir 作为引用 skill。
...
- 2026-03-01：`build` 新增 `--all`，可将当前目录下所有合法 skillDir 批量写为引用 skill。
